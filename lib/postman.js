...
module.exports = {
  generate: function(collectionOld, options = {filter: null, headers: null, output: null}, cb = null) {
    var filter = options.filter;

    if (filter && filter.length > 0) {
      var collectionOld = new postman.Collection(collectionOld);
      var collection = new postman.Collection(this.getRawCollection(options.output));

      collection = this.setItems(collectionOld, collection, filter, options.headers);
    } else {
      collection = new postman.Collection(collectionOld);
    }

    var collectionJson = JSON.stringify(collection, null, 2);

    fs.writeFile(options.output, collectionJson, function(err){
      console.log(options.output);

      if (err) {
        cb(err);
      } else {
        cb(null, options.output);
      }
    });
  },
